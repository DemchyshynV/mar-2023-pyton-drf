...
class ChatConsumer(GenericAsyncAPIConsumer):
    ...
    async def connect(self):
        ...
        await self.send_last_five_messages()
        await self.channel_layer.group_send(
            self.room_name,
            {
                'type': 'sender',
                'body': 'connected to chat',
                'user': self.user_name
            }
        )
    ...
    async def send_last_five_messages(self):
        messages = await self.get_last_file_messages()
    ...
    @database_sync_to_async
    def get_last_file_messages(self):
